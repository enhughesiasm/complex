...
const BoxEmployees = (props) => {
	...
	const {employeesUnlocked, employeesCanHire, employeesGetCost, employeesHire, employees } = props;

	// let heightUnitsPerEmployee = 14;
	// let height = 2 + (Math.max(employeesUnlocked.length, 1) * heightUnitsPerEmployee);

	return(<div className="tile is-child homeBox box employees cardList">
		...
		{ employeesUnlocked.map(emp => {
			...
			let pauseIcon = emp.work.paused ? 'play-circle' : 'pause-circle';
			let pauseText = emp.work.paused ? 'Order them back to work!' :'Ask them to stop work, for some reason.';

			let workdescription = emp.type == employeeTypes.PRODUCTION ? 'Making traits by hand: ' :
				(emp.type == employeeTypes.DELIVERY ? 'Delivering traits: ' : 'Building machines: ');

			let employeeStatus = employees[emp.type];

			return(

				<ComplexListEntry key={emp.id}
					id={emp.id}
					...
					tooltipData={workdescription + (emp.outcomePerSecond || 0).toFixed(2) + ' per second'}
					purchaseText="Hire"
					// employee specific:
					showProgressBar={showProgressBar}
					progressTooFast={emp.currentTimeTaken < .9}
					progress={emp.work.progress}
					workRequiredToComplete={emp.workRequiredToComplete * employeeStatus.workRequiredMultiplier}
					paused={emp.work.paused}
					onPauseClicked={() => emp.work.paused = !emp.work.paused}
					pauseText={pauseText}
					pauseIcon={pauseIcon}
