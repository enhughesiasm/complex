...
const machineUpgrades = [
	{
		...
		'description': 'If machines are good, then better machines must be even better...',
		...
		apply: function(traitsState){
			traitsState.machines.all.filter(a => a.tier == 1).forEach(m => m.unlocked = true);
			traitsState.getUpgradeByKey(upgradesKeys.EVEN_BETTER_MACHINERY).unlocked= true;
			traitsState.getUpgradeByKey(upgradesKeys.FLEXIBLE_MACHINERY).unlocked= true;
		}
	},
	{
		key: upgradesKeys.FLEXIBLE_MACHINERY,
		'name': 'Flexible Machinery',
		'description': 'Teach machines to modify themselves so they can perform different tasks.',
		'explanation': 'Unlocks the ability to move factory capacity between production and delivery.',
		icon: 'exchange-alt',
		cost: 150000,
		unlocked: false,
	...
	},
	{
		...
		apply: function(traitsState){
			traitsState.machines.all.filter(a => a.tier == 2).forEach(m => m.unlocked = true);
			traitsState.getUpgradeByKey(upgradesKeys.CHEAPER_MATERIALS).unlocked= true;
			traitsState.getUpgradeByKey(upgradesKeys.EVEN_BETTERER_MACHINERY).unlocked= true;
		}
	},
	{
		key: upgradesKeys.CHEAPER_MATERIALS,
		'name': 'Industrial Quarrying',
		'description': 'Pull metal straight out of the ground so all your machines will be cheaper.',
		...
		unlockedAtTraitsDelivered: 450000,
		apply: function(traitsState){
			traitsState.machineCostMultiplier *= .75;
			traitsState.getUpgradeByKey(upgradesKeys.MORE_EFFICIENT_MACHINERY).unlocked= true;
			traitsState.getUpgradeByKey(upgradesKeys.SMARTER_CONSTRUCTION_METHODS).unlocked= true;
		}
	},
	{
		key: upgradesKeys.MORE_EFFICIENT_MACHINERY,
		'name': 'More efficient machinery',
		'description': 'Your old designs were wasteful, but now you see how they can be GREATLY improved.',
	...
	},
	{
		...
		apply: function(traitsState){
			...
			traitsState.employees[employeeTypes.BUILDER].workPerSecondMultiplier *= 1.25;
		}
	},
	{
		...
		apply: function(traitsState){
			traitsState.machines.all.filter(a => a.tier == 3).forEach(m => m.unlocked = true);
			traitsState.getUpgradeByKey(upgradesKeys.EVEN_BETTERERER_MACHINERY).unlocked= true;
		}
	},
	{
		...
		apply: function(traitsState){
			traitsState.machines.all.filter(a => a.tier == 4).forEach(m => m.unlocked = true);
			traitsState.getUpgradeByKey(upgradesKeys.UNIMAGINABLE_MACHINERY).unlocked= true;
		}
	},
	{
		...
		apply: function(traitsState){
			traitsState.machines.all.filter(a => a.tier == 5).forEach(m => m.unlocked = true);
			traitsState.getUpgradeByKey(upgradesKeys.SELF_REPLICATING_MACHINES).unlocked= true;
		}
	},
	{
		key: upgradesKeys.SELF_REPLICATING_MACHINES,
		'name': 'Self-Replicating Machines',
		'description': 'Speed up construction by teaching machines to build themselves.',
